...
circos_links = circos_links %>% mutate(color_ligand_type = "#B4EEB4") %>% mutate(color_target_type = "skyblue")
...
gaps = c(
  rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(color_ligand_type == "#B4EEB4") %>% distinct(ligand) %>% nrow() -1)),
...
pdf("plot/NicheNet_CD4_Neutrophils_Receptor_circos.pdf",height = 9,width = 9)
circos.par(gap.degree = gaps)
...
# For Figure 5b in the Article
pdf("plot/NicheNet_CD4_Neutrophils_Target_circos.pdf",height = 9,width = 9)
circos.par(gap.degree = gaps)
chordDiagram(links_circle, directional = 1,order=order,
             link.sort = TRUE, link.decreasing = FALSE, 
             grid.col = grid_col,transparency = transparency, 
