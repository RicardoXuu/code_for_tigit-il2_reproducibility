...
active_ligand_target_links_df = nichenet_output$ligand_target_df %>% mutate(target_type = "CD8+T cells") %>% mutate(ligand_type = "Neutrophils")
...
circos_links = circos_links %>% mutate(color_ligand_type = "skyblue") %>% mutate(color_target_type = "tomato")
...
gaps = c(
  rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(color_ligand_type == "skyblue") %>% distinct(ligand) %>% nrow() -1)),
  width_different_cell,
  rep(width_same_cell_same_target_type, times = (circos_links %>% filter(color_target_type == "tomato") %>% distinct(receptor) %>% nrow() -1)),
  width_different_cell
)
...
# For Extended Data Fig.7c in Article
pdf("plot/NicheNet_Neutrophils_CD8_Receptor_circos.pdf",height = 9,width = 9)
circos.par(gap.degree = gaps)
...
# For Extended Data Fig.7d in Article
pdf("plot/NicheNet_Neutrophils_CD8_Target_circos.pdf",height = 9,width = 9)
circos.par(gap.degree = gaps)
chordDiagram(links_circle, directional = 1,order=order,
             link.sort = TRUE, link.decreasing = FALSE, 
             grid.col = grid_col,transparency = transparency, 
