...
Cluster <- read.table("data/Analysis/4_Cell_clustering_analysis/4_1_all_cell_cluster/all_cells_features.csv",
...
AnnoInfo <- read.table("data/Analysis/8_Reanno/all_cells_features.csv", row.names = 1, header = TRUE, sep = ",")
seurat@meta.data$anno <- AnnoInfo$clusters

rm(Cluster, AnnoInfo)

# Data normalization and subsetting
...
# GO enrichment for macrophage cluster 5
...
ego_bp_macrophage_5 <- enrichGO(
  gene          = DEGs_macrophage_5$ensembl,
  OrgDb         = org.Mm.eg.db,
  keyType       = 'ENSEMBL',
  ont           = "BP",
  pAdjustMethod = "BH",
...
# Define original p.adjust breakpoints and labels
...
# GO enrichment for macrophage cluster 13
...
ego_bp_macrophage_13 <- enrichGO(
  gene          = DEGs_macrophage_13$ensembl,
  OrgDb         = org.Mm.eg.db,
  keyType       = 'ENSEMBL',
  ont           = "BP",
  pAdjustMethod = "BH",
