...
# Analysis of ligand-receptor interactions in mouse T2 vs PBS from Treg cells to CD8+T cells and CD4+Tconv cells using NicheNet
rm(list = ls())
...
active_ligand_receptor_num <- table(active_ligand_receptor_links_df$ligand)[table(active_ligand_receptor_links_df$ligand) >= 3]

active_ligand_receptor_links_df <- active_ligand_receptor_links_df[active_ligand_receptor_links_df$ligand%in%active_ligand_receptor_max$ligand[active_ligand_receptor_max$weight >= 1] 
...
active_ligand_target_links_df = nichenet_output$ligand_target_df %>% mutate(target_type = "CD4+Tconv cells") %>% mutate(ligand_type = "Treg cells")
...
circos_links = circos_links %>% mutate(color_ligand_type = "orange") %>% mutate(color_target_type = "#B4EEB4")
circos_links = circos_links %>% filter(!grepl("^H2", ligand))
circos_links = circos_links %>% filter(!grepl("B2m", ligand))


target_weight_sum = aggregate(weight ~ target, data = circos_links, FUN = sum)
target_weight_sum = target_weight_sum[order(target_weight_sum$weight, decreasing = TRUE),]
rownames(target_weight_sum) <- 1:nrow(target_weight_sum)
target_save = target_weight_sum$target[target_weight_sum$weight >= 0.2]
circos_links = circos_links %>% filter(target %in% target_save)
...
gaps = c(
  ...
  rep(width_same_cell_same_target_type, times = (circos_links %>% filter(color_target_type == "#B4EEB4") %>% distinct(target) %>% nrow() -1)),
  width_different_cell
)
...
pdf("plot/NicheNet_Treg_CD4_Target_circos_T2vsPBS_Up.pdf",height = 9,width = 9)
circos.par(gap.degree = gaps)
...
write.csv(circos_links, "data/NicheNet_Treg_CD4_Target_circos_T2vsPBS_Up.csv", row.names = FALSE)
