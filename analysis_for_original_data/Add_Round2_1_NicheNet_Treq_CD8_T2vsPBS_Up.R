...
active_ligand_target_links_df = nichenet_output$ligand_target_df %>% mutate(target_type = "CD8+T cells") %>% mutate(ligand_type = "Treg cells")
...
circos_links = circos_links %>% mutate(color_ligand_type = "orange") %>% mutate(color_target_type = "tomato")
...
target_save = target_weight_sum$target[target_weight_sum$weight >= 0.1]
...
gaps = c(
  ...
  rep(width_same_cell_same_target_type, times = (circos_links %>% filter(color_target_type == "tomato") %>% distinct(target) %>% nrow() -1)),
  width_different_cell
)
...
pdf("plot/NicheNet_Treg_CD8_Target_circos_T2vsPBS_Up.pdf",height = 9,width = 9)
circos.par(gap.degree = gaps)
...
write.csv(circos_links, "data/NicheNet_Treg_CD8_Target_circos_T2vsPBS_Up.csv", row.names = FALSE)
